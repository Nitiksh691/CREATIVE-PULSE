          ...
          >
            <defs>
              ...
              <linearGradient id="fillProfileViews" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
                  stopColor="var(--color-profile_views)"
                  stopOpacity={0.8}
                />
                <stop
                  offset="95%"
              ...
              <linearGradient id="fillSavedJobs" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
                  stopColor="var(--color-saved_jobs)"
                  stopOpacity={0.8}
                />
                <stop
                  offset="95%"
            ...
            <Area
              dataKey="saved_jobs"
              type="linear"
              fill="url(#fillSavedJobs)"
              fillOpacity={0.4}
              stroke="var(--color-saved_jobs)"
              strokeWidth={2}
              dot={false}
              activeDot={{ r: 4 }}
            />
  ...
  return (
    ...
    >
      ...
      <TabsContent value="week" className="space-y-4">
        {renderChart(mockData.chartData.week.map(d => ({ date: d.date, applications: d.spendings, profile_views: d.sales, saved_jobs: d.coffee })))}
      </TabsContent>
      <TabsContent value="month" className="space-y-4">
        {renderChart(mockData.chartData.month.map(d => ({ date: d.date, applications: d.spendings, profile_views: d.sales, saved_jobs: d.coffee })))}
      </TabsContent>
      <TabsContent value="year" className="space-y-4">
        {renderChart(mockData.chartData.year.map(d => ({ date: d.date, applications: d.spendings, profile_views: d.sales, saved_jobs: d.coffee })))}
      </TabsContent>
    </Tabs>
  );
}
